...
class User(Base):
    ...
    beta_variant: Mapped[str | None] = mapped_column(String(50), nullable=True)
    beta_welcome_seen_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    beta_card_walkthrough_completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
